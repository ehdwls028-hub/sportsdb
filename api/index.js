...
const { initDB, getTeams, getArticleCountByTeam, getArticleCountByTeamId, getArticlesByTeam, getAllArticles, getCrawlLogs, getLastCrawlStatus, getTotalStats, createPost, getAllPosts, getPost, updatePost, deletePost } = require('../src/database');
const { KBO_CHANNELS, getLatestVideos, getChannelInfo, getShorts } = require('../src/youtube');
...
app.get('/', async (req, res) => {
  try {
    ...
    const [sportsdbVideos, ...teamVideosArr] = await Promise.allSettled([
      getLatestVideos(sportsdbId, 15), ...KBO_CHANNELS.map(t => getLatestVideos(t.channelId, 5))
    ]);
    ...
    res.render('home', { title: TITLE, activeTab: 'home',
    ...
    res.render('home', { title: TITLE, activeTab: 'home', sportsdbVideos: [],
      teams: getTeams(), teamCounts: getArticleCountByTeam(), allArticles: getAllArticles(999, 0), teamYoutubeVideos: [] });
  }
});

app.get('/sportsdb', async (req, res) => {
  try {
    const cid = 'UCMStj0Bzmmf1frzu4WgJq2w';
    const [ci, v, sh] = await Promise.all([getChannelInfo(cid), getLatestVideos(cid, 15), getShorts(cid, 20)]);
    res.render('sportsdb', { title: TITLE, activeTab: 'sportsdb', channelInfo: ci, videos: v, shorts: sh });
  } catch (e) {
...
app.get('/press', (req, res) => {
  ...
  const perPage = 10, offset = (page - 1) * perPage;
  let articles = [], totalCount = 0, totalPages = 1, currentPage = page;
  if (selectedTeam === 'all') {
    articles = getAllArticles(999, 0); totalCount = articles.length;
  } else {
...
app.post('/api/articles', (req, res) => {
  upload.single('image')(req, res, function(err) {
    if (err) return res.json({ success: false, message: err.code === 'LIMIT_FILE_SIZE' ? '파일이 너무 큽니다. 최대 10MB' : err.message });
    if (!req.body.title) return res.json({ success: false, message: '제목을 입력하세요.' });
    res.json({ success: true, id: createPost(req.body.title, req.body.content, req.file ? '/uploads/' + req.file.filename : null) });
  });
});
...
app.delete('/api/posts/:id', (req, res) => { deletePost(req.params.id); res.json({ success: true }); });
...
app.post('/admin/articles/:id/edit', upload.single('image'), (req, res) => {
  if (!getPost(req.params.id)) return res.redirect('/admin');
  updatePost(req.params.id, req.body.title, req.body.content, req.file ? '/uploads/' + req.file.filename : null);
  res.redirect('/admin');
});

app.get('/articles', (req, res) => res.render('articles', { title: TITLE, activeTab: 'articles', articles: getAllPosts() }));
...
app.get('/admin', (req, res) => {
  res.render('admin', { title: TITLE, activeTab: '', stats: getTotalStats(), teamStatus: getLastCrawlStatus(), logs: getCrawlLogs(50) });
});
